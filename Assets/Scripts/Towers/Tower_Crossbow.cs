...
public class Tower_Crossbow : Tower {
    ...
    /// <summary>
    /// Unity Awake method for caching the Tower_Crossbow_Visuals component.
    /// </summary>
    ...
    /// <summary>
    /// Overrides the Attack method to perform a raycast attack, rotate the tower head towards the enemy, and play visual effects.
    /// </summary>
    protected override void Attack() {
        ...
        if (Physics.Raycast(gunPoint.position, directionToEnemy, out RaycastHit hitInfo, attackRange, LayerMask.GetMask("Enemy"))) {
            // If the raycast hits the current enemy, set the tower head to face the enemy.
            towerHead.forward = directionToEnemy;
            // If the raycast hit matches the current enemy, execute the attack visuals.
            if (hitInfo.transform == currentEnemy) {
                Debug.DrawRay(gunPoint.position, directionToEnemy * attackRange, Color.red, 1f);
                // Uncomment the line below to see the raycast hits in the editor.
